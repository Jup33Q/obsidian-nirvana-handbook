...
> **关键**：`phx-update="ignore"` 防止 LiveView DOM diff 覆盖 Three.js 渲染的 canvas。citeweb_search:4#3
...
LiveFlip 通过 Web Animations API 实现 FLIP（First, Last, Invert, Play）动画，让元素在位置/尺寸变化时平滑过渡。citeweb_search:4#0
...
LiveMotion 提供类似 Framer Motion 的声明式动画 API，支持 mount/unmount 动画。citeweb_search:4#17
...
使用 `Phoenix.LiveView.JS` 的 `transition/3` 和 `show/hide` 函数实现。citeweb_search:6#2web_search:6#5
...
> **注意**：`JS.show` 配合 `style="display: none"` 和初始 class 是实现可靠进入动画的正确方式。`JS.transition` 单独使用时在 LiveView 的 DOM patching 机制下可能时机不对。citeweb_search:6#8

### 7.4 方案 D: 自定义 Hook 实现完整页面过渡 (SPA 风格)

监听 LiveView 的 `phx:page-loading-start` 和 `phx:page-loading-stop` 事件，实现类似 SPA 的页面切换动画。citeweb_search:6#4
...
3. **LiveView 优化**：
   ...
   - 使用 `JS` 命令的 `blocking: false` 选项防止动画阻塞交互citeweb_search:4#2
...
- LiveFlip (FLIP 动画): https://github.com/probably-not/live-flip citeweb_search:4#0
- LiveMotion (声明式动画): https://hexdocs.pm/live_motion/ citeweb_search:4#17
- LiveView 页面过渡: https://www.mave.io/blog/page-transitions-with-phoenix-liveview/ citeweb_search:4#1web_search:6#0
- LiveView JS 过渡: https://alembic.com.au/blog/improve-ux-with-liveview-page-transitions citeweb_search:4#6web_search:6#4
- LiveView.JS 文档: https://hexdocs.pm/phoenix_live_view/Phoenix.LiveView.JS.html citeweb_search:6#2
- Shared Element Transitions API: https://www.smashingmagazine.com/2022/10/ui-animations-shared-element-transitions-api-part1/ citeweb_search:6#3
